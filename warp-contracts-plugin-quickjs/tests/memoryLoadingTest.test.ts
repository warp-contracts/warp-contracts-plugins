...
describe('Memory loading test', () => {
    ...
    test('should properly use PRNG', async () => {
        ...
        const result1 = await quickJs.handle(messageRandom1, processEnv);
        ...
        const result2 = await quickJs.handle(messageRandom2, processEnv);
    ...
    test('should respect passed current state', async () => {
        ...
        let result;
        result = await quickJs2.handle(message, processEnv, contractState);
        contractState = result.State;
        result = await quickJs2.handle(message, processEnv, contractState);
        contractState = result.State;
        result = await quickJs2.handle(message, processEnv, contractState);
        contractState = result.State;
        const result2 = await quickJs2.handle(message, processEnv, contractState);
        expect(result2.Messages[0].Tags.find((t: {
            name: string;
            value: string
        }) => t.name == 'counter').value).toEqual(4);
    ...
    test('should properly set process and module data in ao.env', async () => {
        console.log("aaa")
        const quickJs = await quickJSPlugin.process({
            contractSource,
            binaryType: "release_sync"
        ...
        expect(result.Output).toEqual({
            ...
            Module: {
                Id: 'yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy',
                Owner: 'zenon',
                Tags: []
            }
        });
